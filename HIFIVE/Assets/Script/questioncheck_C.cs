/*****************
* 프로그램명 : questioncheck_C.cs

* 작성자 : 권순규, 천은정 ( 김성수, 유병주, 전승원 )

* 작성일 : 2019년 12월 7일

* 프로그램 설명 : city맵의 아이트래커와 키넥트를 이용해 퀴즈의 정답을 체크하는 코드입니다.
...
public class questioncheck_C : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (checkin)
        {
            ...
            {
                if (historyquestion.Q[historyquestion.num].check.Equals(check1))
                {
                    Run_City.check = 1;
        ...
        if (checkin)
        {
            ...
            {
                if (historyquestion.Q[historyquestion.num].check.Equals(check2))
                {
                    Run_City.check = 1;
                    GameObject.Find("GameScreen").transform.Find("o").gameObject.SetActive(true);
                    GameObject.Find("GameScreen").transform.Find("Panel").gameObject.SetActive(false);
        ...
        if (checkin)
        {
            ...
            {
                if (historyquestion.Q[historyquestion.num].check.Equals(check3))
                {
                    Run_City.check = 1;
                    GameObject.Find("GameScreen").transform.Find("o").gameObject.SetActive(true);
                    GameObject.Find("GameScreen").transform.Find("Panel").gameObject.SetActive(false);
